---
graphify_id: base-out::supabase_functions_material_download_index
source_file: supabase/functions/material-download/index.js
node_type: code
tags: [su2qc, graphify]
---

# material-download/index.js

Purpose: Graphify extracted this code from the repository.

Graphify node ID: `base-out::supabase_functions_material_download_index`
Repository source: `supabase/functions/material-download/index.js`
Node type: `code`
Source commit: `05842ad04328b434f25b46e49d4eb28d7c0c8abe`

## Important symbols

- material-download/index.js

## Incoming dependencies

- None recorded.

## Outgoing dependencies

- [[base-out_supabase_functions_shared_download_handler_creatematerialdownloadhandler]]
- [[base-out_supabase_functions_material_download_index_adminclient]]
- [[base-out_supabase_functions_material_download_index_userclient]]
- [[base-out_supabase_functions_shared_download_handler]]
- [[base-out_supabase_functions_material_download_index_url]]
- [[base-out_supabase_functions_material_download_index_anonkey]]
- [[base-out_supabase_functions_material_download_index_servicekey]]

## Responsibilities and security relevance

Runtime responsibilities and security relevance are limited to what Graphify and the repository source establish; dynamic behavior remains subject to the execution reports.

Related map: [[SU2QC Codebase Map]]
