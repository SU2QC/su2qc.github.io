...
export function formatCitation(c) {
  const authors = (c.authors || []).map(author => {
    if (author.includes(",")) {
  ...
  const venue = c.journal || c.booktitle || c.publisher || "";
  const locator = c.doi ? `doi:${c.doi}` : c.url;
  return [authors, `“${c.title}.”`, venue, c.volume && `vol. ${c.volume}`, c.pages && `pp. ${c.pages}`, c.year, locator].filter(Boolean).join(" ");
}
